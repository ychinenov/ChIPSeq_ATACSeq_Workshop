...
```{r, fig.align = "left", fig.height=2,fig.width=2}
par(mfrow=c(1,1))
venn <- makeVennDiagram(rep_ol,totalTest=1000, scaled=T, NameOfPeaks=c("GR_Dex_rep1", "GR_Dex_rep2"), fill=c("blue","cyan"),cex=1.5, cat.cex=1,fontface="bold", fontfamily="Arial",cat.fontface="bold",cat.fontfamily="Arial",main="GR_Dex, rep1 versus rep2")
```
...
as created this table does not references peak names. if you want names to be added you can create them from  `peak_u` as following
`count_tab$peakID <- paste0(peak_u@seqnames,":",peak_u@ranges@start,"-",(peak_u@ranges@start+peak_u@ranges@width-1))`
you would have to adjust `ggpairs` count_tab accordingly to exclude peakID column from calculations - `count_tab[,1:(ncol(count_tab)-1)]`
```{r}   
...
The function needs to know where to count reads, in our case we want to use `peak_u` GRange that contains the union of peaks for the hormone treated macrophages  for both replicas (**window=peak_u**). **type=** specifies the type of target file - bam or bigWig.
...
```
`getSeq` returns an object of class `DNAStringSet` (Biostrings package).If we want to export this object as a fasta file  we need to provide names for individual sequences. We will use the same approach as before - concatenate the chromosome name and start/end coordinates as a sequence names. Names of elements in most R objects are accessed using `names()` and are replaced using `names() <-`. Here I am using  **peak_u** variable to create our name using `paste0()`.  
...
```
This fasta file can be used as an input for motif-predicting software such as MEME-chip (http://meme-suite.org/tools/meme-chip). I did just that. You can access the entire results directory  at  `./appMEMECHIP_4.12.01509807486762-571624097/meme-chip.html` 
...
### Sumarizing the results on a single figure with "grid:arrange"
   ...
   Looks good, but we have a bunch of these tables and we want to create graphic summary of all of them. So, in the end we would like to apply all  the same function (`bar3`) over elements of a list. This is yet another task easily achieved by some `apply` function 
